...
export default function SyncPage() {
  ...
  const [fineSyncComplete, _setFineSyncComplete] = useState<boolean>(false)
      ...
      <Card className="w-full max-w-md bg-gray-800 text-white">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="flex items-center space-x-2">
            {/* Show loader during coarse sync OR when recordMsg is received and fine sync isn't complete */}
            {(syncing || (recordMsg && !fineSyncComplete)) ? (
              <Loader2 className="animate-spin h-5 w-5 text-gray-400" />
            ) : fineSyncComplete ? (
              // You might want a success icon here
              <span className="text-green-500 text-2xl leading-none">&#x2714;</span> // Checkmark icon (Unicode)
            ) : (
            ...
            <span className="text-sm text-gray-300">
              ...
              ) : recordMsg && !fineSyncComplete ? (
                "Fine synchronization in progress…"
              ) : micReady && !recordMsg && !fineSyncComplete ? (
                "Coarse sync complete, awaiting fine sync beacon…"
              ) : fineSyncComplete ? (
          ...
          {minRtt !== null && (
            <div className="text-sm text-gray-200 space-y-1">
              ...
              {/* Display current offset which will become the fine offset after fine sync */}
          ...
          {micReady && !syncing && !recordMsg && !fineSyncComplete && (
            <div className="text-sm text-green-400">
              Microphone access granted. Coarse sync complete. Waiting for server beacon...
            </div>
          )}

          {/* Display fine sync specific status */}
          {recordMsg && !fineSyncComplete && (
